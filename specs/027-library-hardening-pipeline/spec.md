...
- **FR-015k**: The system MUST include a hybrid source discovery mechanism: automated background scans (LLM-assisted, registry-based via FAIRsharing/BioPortal/OBO Foundry) discover candidate neuroscience data element repositories, but ingestion of any new source requires curator approval at the source level (not per-entity). Once a source is approved, all its entities flow through the standard pipeline automatically.
...
- Q: Source discovery trigger? → A: Hybrid — automated background scan discovers candidates, but ingestion requires curator approval at the source level (not per-entity). Once approved, all entities from that source flow through the pipeline automatically.
- Q: Upstream source evolution? → A: Review recent PRs/issues in dandischema (PR #387 LinkML conversion), bids-specification, nwb-schema, openMINDS, aind-data-schema. Adapters must track upstream format changes. DANDI is converting to LinkML (PR #387) which means the DANDI adapter should eventually consume LinkML YAML directly instead of Pydantic introspection.
